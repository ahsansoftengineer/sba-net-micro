...
ALTER PROCEDURE [dbo].[GetAllEmployeeScoreboards]    
    @EmployeeId BIGINT,    
...
BEGIN    
    ...
    CTEScoreboard AS (  
        SELECT  
            ...
            -- New: Get comma-separated OwnerIds from ScoreboardOwner (non-deleted)
			(SELECT STRING_AGG(CAST(OwnerId AS NVARCHAR(MAX)), ',') 
			 FROM (
				 SELECT DISTINCT OwnerId 
				 FROM ScoreboardOwner 
				 WHERE ScoreboardId = S.ScoreboardId AND IsDeleted = 0
			 ) AS DistinctOwners) AS ScoreboardOwners,
        ...
        INNER JOIN ScoreboardOwner SO ON SO.ScoreboardId = S.ScoreboardId AND SO.OwnerId = (SELECT TOP 1 Value FROM @TYPE_EmployeeRoleProfileIds) AND SO.IsDeleted = 0
        WHERE 
            S.ScoreboardTypeId = 1 AND S.IsDeleted = 0  
